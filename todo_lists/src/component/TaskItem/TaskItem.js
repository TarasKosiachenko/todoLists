...
function TaskItem(todo) {
  ...
  return (
    <>
      <li id={todo.todo.id} className={new Date(todo.todo.due_date) < Date.now() ? "overdue" : "" && todo.todo.done ? "done" : ""}>
        <div>
        ...
        <div>
          <input type="checkbox" className={"checkboxTask" && todo.todo.done ? "checked" : ""} />
